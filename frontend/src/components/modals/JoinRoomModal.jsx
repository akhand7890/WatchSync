...
export default function JoinRoomModal({ isOpen, onClose, onSubmit, isLoading, prefillCode = '' }) {
  ...
  const submit = async (data) => {
    try {
      await onSubmit({ ...data, roomId: data.roomId.trim().toUpperCase() })
      reset()
    } catch {
  ...
  return createPortal(
    <AnimatePresence>
      {isOpen && (
        <div key="join-modal-root" className="fixed inset-0 z-50 flex items-center justify-center p-4">
          {/* Backdrop */}
          <motion.div
          ...
          >
            ...
            <form onSubmit={handleSubmit(submit)} className="flex flex-col gap-4 relative z-10">
              ...
              {/* Password / PIN (if private room) */}
              <div className="flex flex-col gap-1">
                <label className="font-[Geist,sans-serif] font-medium text-[13px] text-[#e4beba]">
                  Room Password / PIN (If Private)
                </label>
                <input
                  id="join-password"
                  type="password"
                  placeholder="Enter password if required"
                  className="bg-[#0e0e10] border border-[#27272A] rounded-lg px-4 py-2 font-[Inter,sans-serif] text-[13px] text-[#e5e1e4] placeholder:text-[#e4beba]/40 focus:outline-none focus:border-[#d0bcff] transition-all duration-200"
                  {...register('password')}
                />
              </div>
