...
import { Users, MessageSquare, ListVideo, Copy, Check, LogOut, HelpCircle, UserPlus, Activity } from 'lucide-react'
...
export default function Sidebar() {
  ...
  const watchingCount = new Set(participants.map(p => p.username.toLowerCase().trim())).size || participants.length || 1
      ...
      <div className="p-6 border-b border-[#5b403e]/10 flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="font-[Geist,sans-serif] text-[12px] text-[#e4beba] truncate">
            {host ? `Hosted by ${host.username}` : 'Loading...'} • {watchingCount} watching
          </p>
        </div>
      </div>
