...
import { emitPlay, emitPause, emitSeek, emitSetPlaybackRate } from '../../services/socketService'
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (canControl && room?.roomId) {
                  emitSetPlaybackRate(socket, { roomId: room.roomId, playbackRate: rate })
                }
              }}
              className={`bg-black/60 border border-white/20 text-[12px] font-semibold text-[#e5e1e4] px-2 py-1 rounded-md focus:outline-none font-[Geist,sans-serif] ${canControl ? 'cursor-pointer hover:border-[#ffb3ad]' : 'opacity-60 cursor-not-allowed'}`}
              title={canControl ? "Playback Speed" : "Only host/moderator can change speed"}
            >
              <option value={0.5} className="bg-[#18181b]">0.5x</option>
              <option value={1} className="bg-[#18181b]">1.0x (Normal)</option>
              <option value={1.25} className="bg-[#18181b]">1.25x</option>
              <option value={1.5} className="bg-[#18181b]">1.5x</option>
              <option value={2} className="bg-[#18181b]">2.0x</option>
            </select>
          </div>
