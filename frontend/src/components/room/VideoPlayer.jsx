...
import { FloatingReactionsCanvas } from './FloatingReactions'
...
export default function VideoPlayer() {
  ...
  useEffect(() => {
    ...
    const handleRateChange = ({ playbackRate }) => {
      if (playerRef.current && isReadyRef.current) {
        playerRef.current.setPlaybackRate?.(playbackRate)
        toast(`Playback speed set to ${playbackRate}x`, { icon: '⚡' })
      }
    }

    socket.on(EVENTS.PLAYBACK_RATE_CHANGED, handleRateChange)
    return () => socket.off(EVENTS.PLAYBACK_RATE_CHANGED, handleRateChange)
  }, [socket])
